class Seed
  ...
  def self.iata(number)
    Airport.find(number).iata_code
  end

  def self.seed_flight(min, max, min_time, max_time)
    (1..10).each do |number|
      ...
      Flight.create(
        ...
        aircraft: Aircraft.find(Faker::Number.between(1, 10)),
  ...
  def self.create_models
    ...
    (1..10).each do |number|
      Route.create(
        name: "#{iata(number)}-#{iata(number + 1)}",
        departure_airport: Airport.find(number),
        arrival_airport: Airport.find(number + 1)
      )
      Route.create(
        name: "#{iata(number + 1)}-#{iata(number)}",
    ...
    (1..5).each do
      Aircraft.create(
        tail_number: "5Y-#{Faker::Internet.password(4, 4).upcase}",
    ...
    (1..10).each do |number|
      Airfare.create(
        service_charge_in_dollar: Faker::Number.between(700, 1200),
        route: Route.find(number),
        travel_class: TravelClass.find(1)
      )
      Airfare.create(
        service_charge_in_dollar: Faker::Number.between(520, 690),
        route: Route.find(number),
        travel_class: TravelClass.find(2)
      )
      Airfare.create(
        service_charge_in_dollar: Faker::Number.between(300, 500),
        route: Route.find(number),
    ...
    (1..15).each do |number|
      Airfare.create(
        service_charge_in_dollar: Faker::Number.between(117, 200),
        route: Route.find(number + 54),
        travel_class: TravelClass.find(1)
      )
      Airfare.create(
        service_charge_in_dollar: Faker::Number.between(93, 113),
        route: Route.find(number + 54),
        travel_class: TravelClass.find(2)
      )
      Airfare.create(
        service_charge_in_dollar: Faker::Number.between(60, 90),
        route: Route.find(number + 54),
        travel_class: TravelClass.find(3)
      )
    end

    seed_flight(1, 5, 3.hour, 6.hours)
    seed_flight(6, 10, 1.hour, 2.hours)
  end
end